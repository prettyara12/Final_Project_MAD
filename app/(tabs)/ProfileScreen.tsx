...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (personalInfoVisible) setLocalDraft(profileData);
  ...
  const handleEditPhoto = useCallback(() => {
    Alert.alert('📸 Ubah Foto Profil', 'Pilih sumber foto untuk profil kamu:', [
      { text: 'Batal', style: 'cancel' },
      { text: '📷 Kamera', onPress: () => Alert.alert('Kamera', 'Fitur kamera akan segera tersedia.') },
      { text: '🖼️ Galeri', onPress: () => Alert.alert('Galeri', 'Fitur galeri akan segera tersedia.') },
    ]);
  }, []);
  ...
  const handleUpgradePro = useCallback(() => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Mulai Uji Coba Gratis', onPress: () => Alert.alert('🎉 Berhasil!', 'Uji coba gratis 14 hari Pro telah diaktifkan!') },
      ]
    );
  }, []);
  ...
  const handleStatPress = useCallback((type: string) => {
    if (type === 'rating') {
      Alert.alert('⭐ Detail Peringkat', 'Peringkat kamu: 4.9/5.0\n\n📊 Berdasarkan:\n• 15 sesi yang diselesaikan\n• 12 ulasan dari tutor\n• Tingkat kehadiran 98%\n• Tingkat penyelesaian tugas 95%', [{ text: 'Keren!' }]);
    } else {
      Alert.alert('🔥 Detail Beruntun', 'Beruntun saat ini: 12 hari\n\n📅 Rekap:\n• Beruntun terlama: 18 hari\n• Total hari aktif: 45 hari\n• Rata-rata sesi/hari: 2.3\n\nPertahankan momentummu!', [{ text: 'Semangat!' }]);
    }
  }, []);
  ...
  const handleChangePassword = useCallback(() => {
    Alert.alert('🔐 Ubah Kata Sandi', `Link reset kata sandi telah dikirim ke email:\n${profileData.email}`, [{ text: 'Mengerti' }]);
  }, [profileData.email]);

  const handleToggle2FA = useCallback(() => {
    const newState = !twoFAEnabled;
    setTwoFAEnabled(newState);
    Alert.alert(newState ? '🔒 2FA Diaktifkan' : '⚠️ 2FA Dinonaktifkan', newState ? 'Autentikasi dua faktor berhasil diaktifkan.' : 'Autentikasi dua faktor telah dinonaktifkan.');
  }, [twoFAEnabled]);

  const handleToggleBiometric = useCallback(() => {
    const newState = !biometricEnabled;
    setBiometricEnabled(newState);
    Alert.alert(newState ? '👆 Biometrik Aktif' : 'Biometrik Nonaktif', newState ? 'Login dengan sidik jari atau Face ID telah diaktifkan.' : 'Login biometrik telah dinonaktifkan.');
  }, [biometricEnabled]);

  const handleRolePress = useCallback(() => {
    Alert.alert('🎓 Status Akun', `Nama: ${profileData.name}\nPeran: Pelajar\nUniversitas: ${profileData.university}\nJurusan: ${profileData.major}\nTahun: ${profileData.year}\n\nBergabung sejak: Januari 2024`, [{ text: 'Tutup' }]);
  }, [profileData]);
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.profileCard, { backgroundColor: colors.card, shadowColor: colors.cardShadow }]}>
          ...
          <TouchableOpacity onPress={handleRolePress} activeOpacity={0.7}>
            <View style={[styles.rolePill, { backgroundColor: colors.primaryLight }]}>
              <Text style={[styles.rolePillText, { color: colors.primary }]}>PELAJAR</Text>
            </View>
          </TouchableOpacity>
        ...
        {/* Learning Pulse */}
        <TouchableOpacity style={[styles.pulseCard, { backgroundColor: colors.card }]} onPress={() => router.push('/ProgressScreen' as any)} activeOpacity={0.8}>
        ...
        <View style={styles.sectionWrapper}>
          ...
          <View style={[styles.toggleCardGroup, { backgroundColor: colors.surfaceAlt }]}>
            <View style={[styles.toggleRow, { borderBottomWidth: 0 }]}>
              <TouchableOpacity style={styles.toggleRowLeft} onPress={toggleTheme} activeOpacity={0.7}>
                ...
                <View>
                  ...
                  <Text style={[styles.toggleSublabel, { color: colors.textMuted }]}>{isDark ? 'Nyaman untuk malam hari' : 'Mode siang hari aktif'}</Text>
                </View>
              </TouchableOpacity>
              <Switch
          ...
          <View style={[styles.menuCardGroup, { backgroundColor: colors.card }]}>
            ...
            <TouchableOpacity style={[styles.menuItem, { borderBottomWidth: 0 }]} onPress={handleLogout} activeOpacity={0.6}>
              ...
              <View style={styles.menuTextCol}>
                ...
                <Text style={[styles.menuItemSub, { color: '#F87171' }]}>Keluar dari akunmu</Text>
              </View>
          ...
          <View style={[styles.modalContainer, { backgroundColor: colors.card }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {[
                ...
                { label: 'Email', key: 'email', placeholder: 'Email', keyboardType: 'email-address' },
                { label: 'Telepon', key: 'phone', placeholder: 'No. telepon', keyboardType: 'phone-pad' },
                { label: 'Universitas', key: 'university', placeholder: 'Universitas' },
                { label: 'Jurusan', key: 'major', placeholder: 'Jurusan' },
                { label: 'Angkatan / Tahun', key: 'year', placeholder: 'Contoh: Tahun 3' },
              ].map((field) => (
                <View key={field.key} style={styles.modalField}>
                  <Text style={[styles.modalFieldLabel, { color: colors.textTertiary }]}>{field.label}</Text>
                  <TextInput
                    style={[styles.modalInput, { backgroundColor: colors.inputBg, borderColor: colors.inputBorder, color: colors.text }]}
                    value={(localDraft as any)[field.key]}
                    onChangeText={(v) => setLocalDraft(p => ({ ...p, [field.key]: v }))}
          ...
          <View style={[styles.modalContainer, { backgroundColor: colors.card }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={[styles.securityRow, { borderBottomColor: colors.border }]}>
                ...
                <Switch trackColor={{ false: colors.borderAlt, true: colors.success }} thumbColor="#FFFFFF" value={twoFAEnabled} onValueChange={handleToggle2FA} />
              </View>
              ...
              <View style={[styles.securityRow, { borderBottomColor: colors.border }]}>
                ...
                <Switch trackColor={{ false: colors.borderAlt, true: colors.accent }} thumbColor="#FFFFFF" value={biometricEnabled} onValueChange={handleToggleBiometric} />
              </View>

              <TouchableOpacity style={[styles.securityRow, { borderBottomColor: colors.border }]} activeOpacity={0.6}
              ...
              <TouchableOpacity style={[styles.securityRow, { borderBottomWidth: 0 }]} activeOpacity={0.6}
                onPress={() => Alert.alert('⚠️ Hapus Akun', 'Tindakan ini bersifat PERMANEN.\n\nSemua data belajar, sertifikat, dan riwayat sesi akan dihapus.\n\nApakah kamu yakin?', [
                  { text: 'Batal', style: 'cancel' },
                  { text: 'Hapus Akun', style: 'destructive', onPress: () => Alert.alert('📧 Konfirmasi', 'Email konfirmasi telah dikirim.') },
                ])}
              >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContainer, { backgroundColor: colors.card }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TouchableOpacity style={[styles.subPlanCard, { backgroundColor: colors.card, borderColor: colors.primaryLighter }]} activeOpacity={0.7}
                onPress={() => Alert.alert('⭐ Pro Bulanan', 'Mulai paket Pro Bulanan?\n\nRp 99.000/bulan\nUji coba gratis 14 hari', [
                  { text: 'Batal', style: 'cancel' },
                  { text: 'Mulai Uji Coba', onPress: () => { Alert.alert('🎉', 'Uji coba gratis 14 hari dimulai!'); setSubscriptionVisible(false); } },
                ])}
              >
                ...
                {['Sesi bimbingan tak terbatas', 'AI tutor personal 24/7', 'Analitik mendalam'].map((f) => (
                  <View key={f} style={styles.subPlanFeatureRow}>
                    <Ionicons name="checkmark-circle" size={16} color="#10B981" />
                    <Text style={[styles.subPlanFeatureText, { color: colors.textSecondary }]}>{f}</Text>
                  </View>
                ))}
              </TouchableOpacity>

              <TouchableOpacity style={[styles.subPlanCard, styles.subPlanCardPremium]} activeOpacity={0.7}
              ...
              >
                ...
                {['Semua fitur Pro Bulanan', 'Sertifikat penyelesaian', 'Prioritas pencocokan tutor'].map((f) => (
                  <View key={f} style={styles.subPlanFeatureRow}>
                    <Ionicons name="checkmark-circle" size={16} color="#A5B4FC" />
                    <Text style={[styles.subPlanFeatureText, { color: '#E0E7FF' }]}>{f}</Text>
                  </View>
                ))}
              </TouchableOpacity>

              <TouchableOpacity style={styles.subHistoryBtn} activeOpacity={0.6}
...
const styles = StyleSheet.create({
  ...
  logoImage: {
    width: 44,            // pas ukuran logo bulat
    height: 44,
    borderRadius: 8,
    alignSelf: 'flex-start', // rapat kiri
  },
  ...
  statPill: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, gap: 6 },
  ...
  progressPercentAlt: { fontSize: 12, fontWeight: '700' },
  progBarBg: { height: 8, borderRadius: 4 },
  progBarFill: { height: 8, borderRadius: 4 },
  targetRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, paddingTop: 16, marginTop: 4 },
  targetItalicText: { fontStyle: 'italic', fontSize: 12 },
  ...
  toggleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 16, borderBottomWidth: 1 },
  ...
  toggleSublabel: { fontSize: 10, marginTop: 2 },
  ...
  menuTextCol: { flex: 1 },
  menuItemTitle: { fontSize: 14, fontWeight: '700', marginBottom: 2 },
  menuItemSub: { fontSize: 11 },

  // ── MODAL ───────────────────────────────────────────────────────
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalContainer: { borderTopLeftRadius: 32, borderTopRightRadius: 32, paddingHorizontal: 24, paddingTop: 20, paddingBottom: Platform.OS === 'ios' ? 40 : 24, maxHeight: '85%' },
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 },
  modalTitle: { fontSize: 20, fontWeight: '800' },
  modalField: { marginBottom: 16 },
  modalFieldLabel: { fontSize: 12, fontWeight: '700', marginBottom: 6, letterSpacing: 0.3 },
  modalInput: { borderRadius: 16, paddingHorizontal: 16, paddingVertical: 12, fontSize: 14, borderWidth: 1 },
  modalSaveBtn: { borderRadius: 20, paddingVertical: 14, alignItems: 'center', marginTop: 8, marginBottom: 16, flexDirection: 'row', justifyContent: 'center' },
  modalSaveBtnText: { color: '#FFFFFF', fontWeight: '700', fontSize: 14 },

  // ── SECURITY ────────────────────────────────────────────────────
  securityRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 16, borderBottomWidth: 1, gap: 12 },
  securityIconBox: { width: 40, height: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  securityRowTitle: { fontSize: 14, fontWeight: '700', marginBottom: 2 },
  securityRowSub: { fontSize: 11 },

  // ── SUBSCRIPTION ────────────────────────────────────────────────
  subCurrentPlan: { borderRadius: 20, padding: 20, marginBottom: 16, borderWidth: 1 },
  subPlanBadge: { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8, marginBottom: 8 },
  subPlanBadgeText: { fontSize: 9, fontWeight: '800', letterSpacing: 0.5 },
  subPlanName: { fontSize: 20, fontWeight: '800', marginBottom: 4 },
  subPlanDesc: { fontSize: 12, lineHeight: 18 },
  subPlanCard: { borderRadius: 24, padding: 20, marginBottom: 12, borderWidth: 2 },
  subPlanCardPremium: { backgroundColor: '#4F46E5', borderColor: '#4F46E5' },
  subPlanCardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  subPlanPopularBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8 },
  subPlanPopularText: { fontSize: 9, fontWeight: '800', letterSpacing: 0.5 },
  subPlanCardName: { fontSize: 16, fontWeight: '700', marginBottom: 4 },
  subPlanCardPrice: { fontSize: 24, fontWeight: '900', marginBottom: 12 },
  subPlanCardPriceUnit: { fontSize: 14, fontWeight: '600' },
  subPlanFeatureRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 },
  subPlanFeatureText: { fontSize: 12 },
  subHistoryBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, gap: 8, marginBottom: 16 },
  subHistoryBtnText: { fontSize: 13, fontWeight: '700' },
});